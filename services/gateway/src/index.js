...
async function startMicroservice(port) {
    ...
    app.get("/", async (req, res) => {
        ...
        const videosResponse = await axios.get("http://metadata/api/v1/videos");
    ...
    app.get("/video", async (req, res) => {
        ...
        const videoResponse = await axios.get(`http://metadata/api/v1/video?id=${videoId}`);
    ...
    app.get("/history", async (req, res) => {
        ...
        const historyResponse = await axios.get("http://history/api/v1/history");
    ...
    app.get("/api/video", async (req, res) => {
        ...
        const response = await axios({ // Forwards the request to the video-streaming microservice.
            method: "GET",
            url: `http://video-streaming/api/v1/video?id=${req.query.id}`,
            data: req, 
            responseType: "stream",
        });
